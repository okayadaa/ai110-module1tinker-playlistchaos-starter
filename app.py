...
## Bug fixed here: fixing the index to saved genre and ensuring the options are lowercase to match normalized genres
...
def render_playlist(label, songs):
    ...
    if not songs:
        ...
        return
## The a small issue resolved: empty search will demonstrate all songs or user types something that filter results
...
##Found a bug. The fixed issue would be has the lowercase options
    mode = st.selectbox(
        "Pick from",
    ...
    if st.button("Feeling lucky"):
        ...
        pick = lucky_pick(playlists, mode=mode_map[mode])
        if pick is None:
...
## Fixed a small bug: compute_playlist_stats needs to ensure to calculate the unique total songs, accurate average energy, and hype ratio as percentage
def stats_section(playlists):
    ...
    col2.metric("Hype ratio", f"{stats['hype_ratio']:.1f}%")
    col3.metric("Average energy", f"{stats['avg_energy']:.2f}")
